...
function getSearchIndex() {
  ...
  searchIndex = [];
  const dataDir = path.join(__dirname, '..', 'shochu');
  const indexes = [
    { file: 'search_index.json', site: 'shochu' },
...
★ サイト別デフォルトジャンル：
- このサイトのデフォルトジャンルは「焼酎・泡盛」
- 「おすすめ教えて」「何かいいのある？」のようにジャンルが曖昧な質問は、焼酎・泡盛として回答する
- ユーザーが別ジャンルを明示した場合はそちらで回答する（「日本酒のおすすめ」等）
- 迷った場合は焼酎で回答。余計な提案をしない
